...
function fitToCoords(coordsList, withTransition) {{
  ...
  svg.interrupt();   // cancel any in-flight fit so two transitions can't overlap
  // Instant on phones: iOS Safari leaves ghost copies of animated SVG transforms
  // (looks like duplicated/doubled labels), so snap straight to the fitted view.
  if (withTransition === false || isMobile) svg.call(zoomBehaviour.transform, target);
...
function ctrlFit() {{
  if (geoMode) {{
    ...
    const cx = (x0+x1)/2, cy = (y0+y1)/2;
    // Mobile league pyramids are too tall to show legibly, so they fit to WIDTH
    // and scroll vertically (anchored at the top). The club grid is laid out
    // portrait (see the club view) and fits BOTH dimensions into the band below
    // the header, so nearly every club is visible at once.
    // topChrome + botChrome stays 152 (matches the grid's aspect target) so the
    // fitted size is unchanged; the split sits the grid just under the breadcrumb
    // with comfortable room above the footer/zoom buttons rather than centred low.
    const topChrome = 86, botChrome = 100, bandH = H - topChrome - botChrome;
    let k, ty;
    if (isMobile && isLeagueFit) {{
      k = Math.min(W / (x1-x0), 5);
      const topRoom = 92;
      ty = ((y1-y0)*k > H - topRoom - 16) ? topRoom - k*y0 : H/2 - k*cy;
    }} else if (isMobile) {{
      k = Math.min(W / (x1-x0), bandH / (y1-y0), 5);
      ty = topChrome + bandH/2 - k*cy;
    }} else {{
      k = Math.min(W / (x1-x0), H / (y1-y0), 5);
      ty = H/2 - k*cy;
    }}
    const tx = W/2 - k*cx;
    const target = d3.zoomIdentity.translate(tx, ty).scale(k);
    svg.interrupt();   // cancel any in-flight fit so two transitions can't overlap
    // Instant on phones — iOS Safari ghosts animated SVG transforms (doubled labels).
    if (isMobile) svg.call(zoomBehaviour.transform, target);
    else svg.transition().duration(450).call(zoomBehaviour.transform, target);
  }}
}}
...
function render(nodes) {{
  ...
  if (isDetail) {{
    ...
    if (isClub) {{
      ...
      // Desktop aims for a roughly square block. Phones shape the block to match
      // the (portrait) screen so fitting BOTH dimensions shows nearly every club
      // at once at the largest size that still fits — no horizontal panning.
      const bandH = Math.max(360, H - 186);     // usable height below the header (matches ctrlFit)
      const wTarget = (W <= 700)
        ? Math.max(widest, Math.sqrt(totalW * ROW_H * (W / bandH)))
...
function onViewportChange() {{
  // Only a real width change (rotation) re-fits. Ignore height-only changes
  // (soft keyboard, toolbar) and tiny iOS width jitter so we never double-render.
  if (Math.abs(window.innerWidth - _vpW) < 40) return;
